...
본 프로젝트에서는 은행 고객 데이터를 기반으로 고객 이탈 예측 모델을 구축하고,
이탈에 영향을 미치는 주요 요인을 분석하였습니다.

분석 결과, 본 데이터는 불균형 구조를 가지기 때문에 Accuracy보다
F1 Score를 중심으로 모델을 평가하는 것이 더 적절함을 확인할 수 있었습니다.
그 결과 RandomForest 모델이 가장 안정적인 성능을 보였습니다.

또한 SHAP 분석을 통해 age, products_number, balance와 같은 변수들이
이탈 예측에 중요한 영향을 미치는 것을 확인하였습니다.
특히 age는 높을수록 이탈 확률이 증가하는 경향을 보였고,
products_number는 특정 구간에서 이탈 위험이 증가하는 비선형 패턴이 나타났습니다.

이를 통해 고연령 고객이나 상품 이용 패턴이 특이한 고객을 중심으로
차별화된 고객 유지 전략이 필요하다는 인사이트를 도출하였습니다.
